...
#include "video_async_decode.h"
#include "utils.h"
#include "bmp.h"

VideoAsyncDecode::VideoAsyncDecode() :
...
bool VideoAsyncDecode::Init(const std::string& file_path, const std::string& base_dir) {
  do {
    if (inited_) {
      break;
    }
...
void VideoAsyncDecode::Progress() {
  while (true) {
...
void VideoAsyncDecode::Uint() {
    ...
    LOGI("VideoAsyncDecode stats %s", stats_.toString().c_str());
}

bool VideoAsyncDecode::TryDecSend() {
...
bool VideoAsyncDecode::TryDecRecv() {
