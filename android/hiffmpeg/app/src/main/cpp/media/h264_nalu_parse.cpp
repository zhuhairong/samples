...
bool H264NaluParse::parse(FILE* fp, H264Info& info) {
  if (!fp) {
    return false;
  }
  ...
  std::list<NaluUnit> naluList;
  if (!parse(naluList, buff, ret)) {
    return false;
  }

  for (auto nalu : naluList) {
    NaluInfo naluInfo;
    naluInfo.pos = info.pos;
    naluInfo.size = nalu.data.size();
    naluInfo.mark_size = nalu.mark_size;
    naluInfo.nal_unit_type = nalu.nal_unit_type;
    info.pos += nalu.data.size();
    info.nalus.push_back(naluInfo);
    ...
    if (nalu.nal_unit_type == kNaluTypeSPS) {
      ...
      sps(data, len, info.width, info.height, info.fps);
    }
  }
