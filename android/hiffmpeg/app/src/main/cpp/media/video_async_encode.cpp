...
#include "video_async_encode.h"
#include "utils.h"

VideoAsyncEncode::VideoAsyncEncode() : inited_(false), media_codec_(nullptr), media_format_(nullptr) {
}

VideoAsyncEncode::~VideoAsyncEncode() {
  Uint();
}

bool VideoAsyncEncode::Init(const VideoAsyncEncodeParmas& parmas) {
  do {
    if (inited_) {
      break;
    }
...
void VideoAsyncEncode::Progress() {
  while (true) {
...
void VideoAsyncEncode::Uint() {
  ...
  LOGI("VideoAsyncEncode stats %s", stats_.toString().c_str());
}

bool VideoAsyncEncode::TryEncSend() {
  do {
...
bool VideoAsyncEncode::TryEncRecv() {
  while (true) {
