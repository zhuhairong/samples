...
public:
    ...
    */
     ...
     *    - path 填写 exe 程序（如 QQ音乐）所在的路径，将会启动此程序并只采集此程序的声音。( Windows 平台)
     ...
     *     - 高清大画面：({@link  TRTCVideoStreamTypeBig})
     *     - 辅流（屏幕分享）：({@link  TRTCVideoStreamTypeSub})
     * 如果type 是TRTCVideoStreamTypeBig，则SDK 不在运行原有的视频采集流程，只保留编码和发送能力。
     ...
     *       - Windows、Android 平台目前仅支持传入 TRTCVideoPixelFormat_I420 格式的视频帧
     */
     ...
     *
     * TRTCVideoFrame 推荐如下填写方式（其他字段不需要填写）：
     * - pixelFormat： Windows、Android平台仅支持
     * TRTCVideoPixelFormat_I420，iOS、Mac平台支持TRTCVideoPixelFormat_I420和TRTCVideoPixelFormat_BGRA32
     * - bufferType：仅支持 TRTCVideoBufferType_Buffer。
     * - data：视频帧 buffer。
     * - length：视频帧数据长度，I420 格式下，其值等于：width × height × 3 / 2。
     * - width：视频图像长度。
     ...
     *
     * @param frame 视频数据，支持 I420 格式数据。
     * @note - SDK 内部有帧率控制逻辑，目标帧率以您在 setVideoEncoderParam
     * 中设置的为准，太快会自动丢帧，太慢则会自动补帧。
     *       - 可以设置 frame 中的 timestamp 为 0，相当于让 SDK 自己设置时间戳，但请“均匀”地控制
     * sendCustomVideoData 的调用间隔，否则会导致视频帧率不稳定。
     *       - iOS、Mac平台目前仅支持传入 TRTCVideoPixelFormat_I420 或 TRTCVideoPixelFormat_BGRA32
     * 格式的视频帧
     *       - Windows、Android 平台目前仅支持传入 TRTCVideoPixelFormat_I420 格式的视频帧
     */
     ...
     * @param enablePlayout 是否混入播放 true：混入播放；false：不混入播放，默认值：false
     * @note enablePublish = false， enablePlayout = false
     * 时，表示完全关闭这个额外的音频流，即不推流，也不播放
     */
     ...
     * - timestamp：如果 timestamp 间隔不均匀，会严重影响音画同步和录制出的 MP4 质量。
     *
     * @param frame 音频帧，仅支持 TRTCAudioFrameFormatPCM
     * 格式。目前只支持单声道，仅支持48K采样率，TRTCAudioFrameFormatPCM 格式。
     * @note 可以设置 frame 中的 timestamp 为 0，相当于让 SDK 自己设置时间戳，但请“均匀”地控制
